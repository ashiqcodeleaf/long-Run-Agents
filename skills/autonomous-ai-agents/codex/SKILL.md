...
metadata:
  myagent:
    ...
    related_skills: [claude-code, myagent]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the MyAgent terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For MyAgent itself, `model.provider: openai-codex` uses MyAgent-managed Codex
OAuth from `~/.myagent/auth.json` after `myagent auth add openai-codex`. For the
...
## MyAgent Gateway Caveat

When invoking the Codex CLI from a MyAgent gateway/service context (for example,
