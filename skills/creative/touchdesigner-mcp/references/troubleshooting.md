...
## 5. MyAgent Configuration

### Config location

`$MYAGENT_HOME/config.yaml` (defaults to `~/.myagent/config.yaml` when `MYAGENT_HOME` is unset)
...
Restart the MyAgent session for changes to take effect. The MCP connection is
