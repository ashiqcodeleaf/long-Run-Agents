...
metadata:
  myagent:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, myagent-video]
...
```
MyAgent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${MYAGENT_HOME:-$HOME/.myagent}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to MyAgent config (if missing)
...
3. **Restart MyAgent session** to pick up the new MCP server
