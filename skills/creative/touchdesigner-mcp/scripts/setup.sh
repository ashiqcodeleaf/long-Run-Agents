...
MYAGENT_HOME_DIR="${MYAGENT_HOME:-$HOME/.myagent}"
MYAGENT_CFG="${MYAGENT_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure MyAgent config has twozero_td MCP entry ──
if [[ ! -f "$MYAGENT_CFG" ]]; then
    echo -e " ${FAIL} MyAgent config not found at ${MYAGENT_CFG}"
    manual_steps+=("Create ${MYAGENT_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$MYAGENT_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in MyAgent config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to MyAgent config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$MYAGENT_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${MYAGENT_CFG} manually"); }
    manual_steps+=("Restart MyAgent session to pick up config change")
fi
