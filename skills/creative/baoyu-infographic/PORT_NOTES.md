...
| Change | Upstream | MyAgent |
|--------|----------|--------|
| Metadata namespace | `openclaw` | `myagent` |
| Trigger | `/baoyu-infographic` slash command | Natural language skill matching |
| User config | EXTEND.md file (project/user/XDG paths) | Removed — not part of MyAgent infra |
...
| File operations | Bash commands | MyAgent file tools (write_file, read_file) |
...
Reference files can be overwritten directly (they're unchanged from upstream). SKILL.md must be manually merged since it contains MyAgent-specific adaptations.
