#!/usr/bin/env python3
"""Google Workspace API CLI for MyAgent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing MyAgent-facing JSON contract and falls back to the Python client
...
from _myagent_home import get_myagent_home
...
def _gws_binary() -> str | None:
    override = os.getenv("MYAGENT_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for MyAgent")
