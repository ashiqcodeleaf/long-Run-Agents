#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for MyAgent.
...
# Ensure sibling modules (_myagent_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _myagent_home import display_myagent_home, get_myagent_home

MYAGENT_HOME = get_myagent_home()
TOKEN_PATH = MYAGENT_HOME / "google_token.json"
CLIENT_SECRET_PATH = MYAGENT_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = MYAGENT_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same MyAgent profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the MyAgent Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the MyAgent Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  pip install 'myagent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to MyAgent home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_myagent_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for MyAgent")
