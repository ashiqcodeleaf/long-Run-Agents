...
description: "Operate the Teams meeting summary pipeline via MyAgent CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions."
version: 1.1.0
...
Everything operator-facing is a `myagent teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${MYAGENT_HOME:-~/.myagent}/.env`:
...
```bash
myagent teams-pipeline validate              # config snapshot — run first after any change
myagent teams-pipeline token-health          # Graph token status
myagent teams-pipeline token-health --force-refresh   # force a fresh token acquisition
myagent teams-pipeline list                  # recent meeting jobs
myagent teams-pipeline list --status failed  # only failed jobs
myagent teams-pipeline show <job-id>         # full detail of one job
myagent teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
myagent teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
myagent teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
myagent teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
myagent teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

myagent teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
myagent teams-pipeline delete-subscription <sub-id>
myagent teams-pipeline maintain-subscriptions            # renew near-expiry ones
myagent teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `myagent teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `myagent cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
