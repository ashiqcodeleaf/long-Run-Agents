...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _myagent_env="${MYAGENT_HOME:-$HOME/.myagent}/.env"; [ -f "$_myagent_env" ] && grep -q "^GITHUB_TOKEN=" "$_myagent_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_myagent_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source "${MYAGENT_HOME:-$HOME/.myagent}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by MyAgent. Code looks clean — good test coverage, no security concerns."
