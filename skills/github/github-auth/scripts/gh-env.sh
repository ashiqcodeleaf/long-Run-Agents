#!/usr/bin/env bash
# GitHub environment detection helper for MyAgent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _myagent_env="${MYAGENT_HOME:-$HOME/.myagent}/.env"; [ -f "$_myagent_env" ] && grep -q "^GITHUB_TOKEN=" "$_myagent_env" 2>/dev/null; then
